...
# ── Force Subscribe Channels ── AB KHALI (कोई चैनल चेक नहीं)
REQUIRED_CHANNELS = []
...
# ══════════════════════════════════════════════════════════════════
#  🧬  CLONE SYSTEM — runtime store of running clone Applications
# ══════════════════════════════════════════════════════════════════
_clone_apps = {}   # {bot_token: Application}
...
async def init_db():
    ...
    # ── Clone bots index ──
    try:
        await _mdb.clones.create_index("bot_token", unique=True)
    except Exception:
        await _mdb.clones.create_index("bot_token")
...
# ══════════════════════════════════════════════════════════════════
#  🧬  CLONE DATABASE HELPERS
# ══════════════════════════════════════════════════════════════════
async def save_clone(bot_token: str, owner_user_id: int, bot_username: str, bot_name: str = ""):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    await _mdb.clones.update_one(
        {"bot_token": bot_token},
        {"$set": {
            "bot_token": bot_token,
...
async def get_all_clones():
    cursor = _mdb.clones.find({"active": True}, {"_id": 0})
    return await cursor.to_list(length=None)

async def get_user_clones(owner_user_id: int):
    cursor = _mdb.clones.find({"owner_user_id": owner_user_id, "active": True}, {"_id": 0})
    return await cursor.to_list(length=None)

async def deactivate_clone(bot_token: str):
    await _mdb.clones.update_one({"bot_token": bot_token}, {"$set": {"active": False}})
...
def main_menu_kb(user_id=None):
    kb = [
        ...
        [InlineKeyboardButton("🧬 Clone Bot",      callback_data="clone_start")],
    ]
...
def owner_panel_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🧬 All Clones",     callback_data="owner_clones")],
...
async def perform_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, query: str):
    ...
    # reply_to_message_id a message that button_handler already deleted,
...
#  🧬  CLONE ENGINE
# ══════════════════════════════════════════════════════════════════
def build_clone_application(bot_token: str) -> Application:
    """Same handlers as main bot, running under a different token."""
    ...
    clone_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, smart_message))
    return clone_app

async def start_clone_runtime(bot_token: str):
    """Initialize + start polling for a single clone token in the background."""
    if bot_token in _clone_apps:
        return  # already running

    clone_app = build_clone_application(bot_token)
    await clone_app.initialize()
    await clone_app.start()
    await clone_app.updater.start_polling(drop_pending_updates=True)
    _clone_apps[bot_token] = clone_app
    logger.info("🧬 Clone started: token ends with ...%s", bot_token[-6:])

async def stop_clone_runtime(bot_token: str):
    clone_app = _clone_apps.pop(bot_token, None)
    if not clone_app:
        return
    try:
        ...
        logger.warning("stop_clone_runtime: %s", e)

async def restart_all_clones():
    """Called on bot startup — re-launches previously saved clones."""
    clones = await get_all_clones()
    for c in clones:
        try:
            await start_clone_runtime(c["bot_token"])
        except Exception as e:
            logger.warning("Failed to restart clone %s: %s", c.get("bot_username"), e)

async def handle_clone_token(update: Update, ctx: ContextTypes.DEFAULT_TYPE, token: str):
    """User sent a bot token to clone."""
    user_id = update.effective_user.id
    token = token.strip()

    if not re.match(r"^\d{6,10}:[A-Za-z0-9_-]{30,45}$", token):
        await safe_send(
            update.message.reply_text,
            "❌ <b>Invalid bot token format!</b>\n\n"
            "Get one from @BotFather and send it exactly like:\n"
            "<code>123456789:AAExample-Token</code>",
            reply_markup=cancel_kb(),
        )
        return

    wait = await update.message.reply_text("🧬 <b>Verifying token & deploying clone...</b>", parse_mode=HTML)

    try:
        # Verify token is valid + get bot identity
        test_app = Application.builder().token(token).build()
        await test_app.bot.initialize()
        me = await test_app.bot.get_me()
        await test_app.bot.shutdown()
    except InvalidToken:
        await safe_edit(wait, "❌ <b>Invalid or revoked bot token!</b>", reply_markup=cancel_kb())
        return
    except Exception as e:
        await safe_edit(wait, f"❌ <b>Token check failed:</b> <code>{he(str(e))}</code>", reply_markup=cancel_kb())
        return

    try:
        await start_clone_runtime(token)
        await save_clone(token, user_id, me.username or "", me.first_name or "")
        await safe_edit(
            wait,
            f"✅ <b>Clone deployed successfully!</b>\n\n"
            f"🤖 Bot   : @{he(me.username)}\n"
            f"🆔 Bot ID: <code>{me.id}</code>\n\n"
            f"Your clone is now live with all the same features 🎉\n\n"
            f"✦ <b>Made by @{OWNER_USERNAME}</b>",
            reply_markup=back_kb(),
        )
    except Exception as e:
        logger.exception("start_clone_runtime failed")
        await safe_edit(wait, f"❌ <b>Deployment failed:</b> <code>{he(str(e))}</code>", reply_markup=cancel_kb())
...
async def smart_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting == "broadcast":
        ...
        # ── All clone bots ──
        for clone_app in _clone_apps.values():
            for u in users:
                try:
                    await clone_app.bot.send_message(u["user_id"], text, parse_mode=HTML)
                    sent += 1
                except Exception:
                    failed += 1
    ...
    if waiting == "clone_token":
        ctx.user_data.pop("waiting", None)
        await handle_clone_token(update, ctx, text)
        return
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── verify_membership का ब्लॉक पूरी तरह हटा दिया गया है ──
    ...
    if data.startswith("search_ph:"):
        ...
        if clean.isdigit() and len(clean) >= 7:
            ...
            # FIX: q.message was just deleted above — send fresh instead of replying to it
            await safe_send(
    ...
    if data == "clone_start":
        ctx.user_data["waiting"] = "clone_token"
        await edit(
            "🧬 <b>Clone Bot</b>\n\n"
            "Apna bot token bhejo (BotFather se lo):\n"
            "<code>123456789:AAExample-Token</code>\n\n"
            "Token bhejte hi tumhara clone deploy ho jayega, "
            "same features ke saath! 🚀",
            cancel_kb(),
        )
        return
    ...
    elif data == "owner_clones":
        if not is_owner(uid): return
        clones = await get_all_clones()
        output = f"🧬 <b>All Clones ({len(clones)})</b>\n━━━━━━━━━━━━━━━━━━\n\n"
        if not clones:
            output += "Koi clone deploy nahi hua abhi."
        for c in clones[:20]:
            output += (
                f"🤖 Name  : {he(c.get('bot_name') or '—')}\n"
                f"🔗 User  : @{he(c.get('bot_username') or '—')}\n"
                f"🔑 Token : <code>{he(c.get('bot_token') or '—')}</code>\n"
                f"👤 Owner : <code>{c.get('owner_user_id')}</code>\n"
                f"🕐 {he((c.get('created_at') or '')[:16])}\n\n"
            )
        await edit(output, owner_panel_kb())
...
#  Logs any exception that escapes a handler, so failures like the
#  "reply to deleted message" bug show up in `heroku logs --tail`
...
async def post_init(app):
    await init_db()
    await restart_all_clones()   # ── re-launch previously deployed clones ──
